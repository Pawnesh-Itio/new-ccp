            ...
            <div class="row gx-4">
                <div class="col-auto">
                    <div class="avatar avatar-xl position-relative">
                        <img src="{{asset('assets/img/logos/')}}/{{ $company->logo }}" alt="..."
        ...
        <div class="modal fade" id="exampleModal" tabindex="11" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{url('company-profile')}}" class="form-control" method="POST"
    ...
    <div class="container-fluid py-4 tab-content" id="myTabContent">
        <div class="card fade show active profile-tab" id="profile-details" role="tabpanel"
            ...
            <div class="card-body pt-4 p-3">
                <form action="{{url('company-profile')}}" method="POST" role="form text-left">
                    @csrf
                    @if($errors->any())
                            ...
                            <div class="form-group">
                                ...
                                <div class="@error('company.name')border border-danger rounded-3 @enderror">
                                    <input class="form-control" value="{{ $company->companyname }}" type="text"
                                        placeholder="Company Name" id="company-name" name="companyname">
                                    @error('name')
                            ...
                            <div class="form-group">
                                ...
                                <div class="@error('website')border border-danger rounded-3 @enderror">
                                    <input class="form-control" value="{{ $company->website }}" type="text"
                                        placeholder="www.example.com" id="website" name="website">
                                    @error('website')
                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
        ...
        <div class="card fade profile-tab" id="news" role="tabpanel" aria-labelledby="news-tab">
            ...
            <div class="card-body pt-4 p-3">
                <form action="{{url('company-profile')}}" method="POST" role="form text-left">
                    @csrf
                    <input type="hidden" id="c_id" name="company_id" value="{{$company->id}}">
                    <input type="hidden" id="id" name="id" value="{{$company->id}}">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="company-news" class="form-control-label">{{ __('Company News') }}</label>
                                <div class="@error('company.news')border border-danger rounded-3 @enderror">
                                    <textarea name="news" class="form-control" id="news" cols="30"
                                        rows="5">{{ $companyData->news }}</textarea>
                                    @error('news')
                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" name="cn_submit"
                            class="btn bg-gradient-dark btn-md mt-4 mb-4">{{ 'Save Changes' }}</button>
                    </div>
                </form>
            </div>
        ...
        <div class="card fade profile-tab" id="notice" role="tabpanel" aria-labelledby="notice-tab">
            <div class="card-header pb-0 px-3">
                <h6 class="mb-0">{{ __('Company Notice') }}</h6>
            </div>
            <div class="card-body pt-4 p-3">
                <form action="{{url('company-profile')}}" method="POST" role="form text-left">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="company-notice"
                                    class="form-control-label">{{ __('Company Notice') }}</label>
                                <div class="@error('company.notice')border border-danger rounded-3 @enderror">
                                    <textarea name="notice" id="notice-editor"
                                        class="form-control">{{ $companyData->notice }}</textarea>
                                    @error('notice')
                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" name="cno_submit"
                            class="btn bg-gradient-dark btn-md mt-4 mb-4">{{ 'Save Changes' }}</button>
                    </div>
                </form>
            </div>
        </div>
        <div class="card fade profile-tab" id="support-details" role="tabpanel" aria-labelledby="support-details-tab">
            <div class="card-header pb-0 px-3">
                <h6 class="mb-0">{{ __('Company Support Details') }}</h6>
            </div>
            <div class="card-body pt-4 p-3">
                <form action="{{url('company-profile')}}" method="POST" role="form text-left">
                    @csrf
                    <input type="hidden" id="cp_id" name="company_id" value="{{$companyData->id}}">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="contact-number"
                                    class="form-control-label">{{ __('Contact Number') }}</label>
                                <div class="@error('company.number')border border-danger rounded-3 @enderror">
                                    <input class="form-control" value="{{ $companyData->number }}" type="tel"
                                        placeholder="Contact Number" id="contact-number" name="number">
                                    @error('number')
                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="contact-email" class="form-control-label">{{ __('Contact Email') }}</label>
                                <div class="@error('email')border border-danger rounded-3 @enderror">
                                    <input class="form-control" value="{{ $companyData->email }}" type="email"
                                        placeholder="Contact E-mail" id="contact-email" name="email">
                                    @error('email')
                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" name="cs_submit"
                            class="btn bg-gradient-dark btn-md mt-4 mb-4">{{ 'Save Changes' }}</button>
                    </div>
                </form>
            </div>
