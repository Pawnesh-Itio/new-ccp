        ...
        <div class="row mt-lg-n10 mt-md-n11 mt-n10">
            <div class="col-xl-8 col-lg-8 col-md-7 mx-auto">
                <div class="card z-index-0">
                    ...
                    <div class="card-body">
                        <form role="form text-left" id="registerForm" action="{{route('registration')}}" method="POST">
                            @csrf
                            <div class="row">
                                <!-- Start Left Col -->
                                <div class="col">
                                ...
                                <div class="col">
                                    ...
                                    <div class="mb-3">
                                        <input type="text" class="form-control" placeholder="Country" name="country"
                                            id="country" aria-label="country" aria-describedby="name"
...
function otpSend(mobile, email) {
    if (mobile.length > 0) {
        $.ajax({
            url: '{{ route("registrationotp") }}',
            type: 'post',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ...
            success: function(data) {
                $('#staticBackdrop').find('.msg').text("OTP have been sent on you email address " + email +"!");
                $('#staticBackdrop').find('input[name="mobile"]').val(mobile);
                $('#staticBackdrop').find('input[name="email"]').val(email);
...
function getCity(state) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // Empty the select field before adding new options
            $('#city').empty();
            ...
            // $('#registerForm').find('.cuntOpt').html(data);
        },
        error: function(xhr, Status, error) {
